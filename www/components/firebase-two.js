...
<button class="enviar-comentario" onclick="enviarComentario(this, '${doc.id}', '${usuarioDoc.id}')">
...
function salvarDocumento(uidDoUsuario, docId) {
  const relatoRef = firebase.firestore().collection('usuarios').doc(uidDoUsuario).collection('relato').doc(docId);
  const relatosSalvosRef = firebase.firestore().collection('usuarios').doc(uidDoUsuario).collection('relatosSalvos').doc(docId);

  relatoRef.get()
    .then((relatoDoc) => {
      if (relatoDoc.exists) {
        const relatoData = relatoDoc.data();

        relatosSalvosRef.set(relatoData)
          .then(() => {
        ...
        console.log('Documento do relato não encontrado.');
      }
    })
    .catch((error) => {
      console.error('Erro ao obter o relato original: ', error);
    });
}
  ...
  firebase.auth().onAuthStateChanged(function(user) {
    if (user) {
      ...
      relatosSalvosRef.set({})
        ...
        .catch((error) => {
          console.error('Erro ao salvar o relato: ', error);
        });
    }
  });
}
...
function removerRelatoSalvo(docId) {
  firebase.auth().onAuthStateChanged(function(user) {
    if (user) {
      const uidDoUsuario = user.uid;
      const usuarioRef = firebase.firestore().collection('usuarios').doc(uidDoUsuario);
      ...
      relatosSalvosRef.delete()
        .then(() => {
          console.log('Relato removido com sucesso');
        })
        .catch((error) => {
          console.error('Erro ao remover o relato salvo: ', error);
        });
    }
  });
}
